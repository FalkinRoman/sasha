...
class SettingsController extends Controller
{
    ...
    public function updateCabinetMode(Request $request, CoursePurchaseService $purchaseService): RedirectResponse
    {
        ...
        $msg = $setting->cabinet_presale_mode
            ? 'Режим: до старта проекта. Карточки уроков (кроме превью) приглушены; срок доступа по тарифу не идёт, пока не включишь «Проект запущен».'
            : 'Проект запущен: обычный кабинет и тарифы.'.($started > 0 ? " Запущен отсчёт доступа (30/60 дн. от сегодня) для {$started} оплаченных покупок без даты ранее." : '');
