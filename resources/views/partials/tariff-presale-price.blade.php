...
@php
    $showPresale = $presaleMode && is_array($pc) && ($pc['discount'] ?? 0) > 0;
@endphp
@if ($showPresale)
    <div class="mt-8 space-y-1.5">
        <div class="flex flex-wrap items-center gap-2">
            @if (! empty($pc['discount_percent']))
                <span class="inline-flex rounded-full bg-[#2d312d] px-2.5 py-1 text-[11px] font-bold tabular-nums tracking-wide text-[#fffffa]" title="Скидка по промокоду предпродажи из админки">−{{ (int) $pc['discount_percent'] }}%</span>
            @endif
