{{-- Кнопка «глаз» для поля пароля; JS: initAuthPasswordToggles() в app.js --}}
@php
    $targetId = $targetId ?? 'password';
@endphp
<button
    type="button"
    class="absolute right-2 top-1/2 inline-flex h-9 w-9 shrink-0 -translate-y-1/2 items-center justify-center rounded-lg text-[#7a837a] transition hover:bg-[#eef0ea] hover:text-[#2d312d] focus:outline-none focus:ring-2 focus:ring-[#869274]/25"
    data-pv-auth-pw-toggle
    data-pv-target-id="{{ $targetId }}"
