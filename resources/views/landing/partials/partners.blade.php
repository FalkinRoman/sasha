...
<section id="partners" class="scroll-mt-24 w-full border-t border-[#ecece8] bg-[#f9faf6] py-16 md:py-24">
    <div class="mx-auto w-full max-w-[720px] px-5 text-center sm:px-8">
        <div data-pv-reveal class="pv-reveal pv-reveal--fade">
            <p class="text-xs font-semibold uppercase tracking-[0.28em] text-[#869274]">{{ $partnersSec?->subtitle ?? 'Вместе выгоднее' }}</p>
            <h2 class="mt-3 text-2xl font-semibold text-[#2d312d] md:text-3xl">{{ $partnersSec?->title ?? 'Приходите вместе' }}</h2>
            ...
            @else
                ...
                <p class="mt-4 text-base leading-relaxed text-[#5c655c]">
                    Промокоды партнёров и скидки — в блоке выше по странице (если подключены).
                </p>
            @endif
            <a href="{{ route('register') }}" class="pv-btn-olive mt-10 inline-flex px-8 py-3.5">Регистрация</a>
        </div>
    </div>
</section>
