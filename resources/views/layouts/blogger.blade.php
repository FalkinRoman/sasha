...
<head>
    ...
    <title>@yield('title', 'Кабинет блогера — ProstoYoga')</title>
...
<body class="app-shell font-sans text-[#2d312d] antialiased">
    ...
    <header class="fixed left-0 right-0 top-0 z-[45] flex items-center justify-between border-b border-[#ecece8] bg-[#fffffa]/95 px-4 py-3 backdrop-blur-md md:hidden">
        <a href="{{ route('blogger.dashboard') }}" class="text-lg font-semibold tracking-tight">Prosto<span class="text-[#869274]">Yoga</span> <span class="text-xs font-normal text-[#7a837a]">блогер</span></a>
        <button
            type="button"
    ...
    <div class="flex min-h-screen flex-col pt-[3.25rem] md:flex-row md:pt-0">
        ...
        >
            <a href="{{ route('blogger.dashboard') }}" class="inline-block text-lg font-semibold transition-opacity duration-300 hover:opacity-85">Prosto<span class="text-[#869274]">Yoga</span></a>
            <p class="mt-1 text-xs text-[#7a837a]">Кабинет блогера</p>
            <nav class="mt-6 flex flex-col gap-1 text-sm">
                <a href="{{ route('blogger.dashboard') }}" class="pv-sidebar-link {{ request()->routeIs('blogger.dashboard') ? 'pv-sidebar-link--active' : '' }}">Курс</a>
                <a href="{{ route('blogger.sales') }}" class="pv-sidebar-link {{ request()->routeIs('blogger.sales') ? 'pv-sidebar-link--active' : '' }}">Продажи по промокоду</a>
